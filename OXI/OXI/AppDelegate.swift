...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func tryToWriteToLibrary()
    {
        ...
        do
        {
            let levelString = Bundle.main.path(forResource: "Files/Level.txt", ofType: "")!
            
            if !fileManager.fileExists(atPath: destinationPath) || rewrite
            {
                //try fileManager.removeItem(atPath: destinationPath)
                try fileManager.copyItem(atPath: levelString, toPath: destinationPath)
                print("DONE")   
            }
